export const featuredItems = {
    'New York': [
        {
            id: 2,
            title: 'Marasca Khao Yai, Khao Yai',
            address: 'หมู่ที่ 5, 87, Tambon Phaya Yen, Amphoe Pak Chong, Chang Wat Nakhon Ratchasima.',
            rooms: 4,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '25,976',
            favourite: false,
            popular: false,
            img: ['/img/Marasca Khao Yai, Khao Yai 1.jpeg', '/img/Marasca Khao Yai, Khao Yai 2.jpeg', '/img/Marasca Khao Yai, Khao Yai 3.jpeg', ]
        },
        {
            id: 3,
            title: 'Basaya Beach Hotel & Resort, Pattaya',
            address: '137 Mu9 Pattaya Klang Road, Nongpreu, Banglamung, Chonburi, Pattaya, Thailand, Chon Buri.',
            rooms: 3,
            type: 'Sale',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: true,
            img: ['/img/Basaya Beach Hotel & Resort, Pattaya 1.png', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg', '/img/featured-img-6.jpg',]
        },
        {
            id: 1,
            title: 'Regal North Hills - 4150 Main at North Hills St, Releigh',
            address: '3890 Poplar Dr.',
            rooms: 3,
            type: 'Sale',
            bed: 4,
            bath: 1,
            area: 732,
            price: '5,256',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-1.jpg']
        },
   
        {
            id: 4,
            title: 'Costco Wholesale - 1021 Oak Forest Ln, Myrtle Beach, SC',
            address: '8558 Parker Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-4.jpg', '/img/featured-img-5.jpg', '/img/featured-img-6.jpg', '/img/featured-img-1.jpg',]
        },
        {
            id: 5,
            title: 'Dollar General - 5416 Rock Quarry Rd, Raleigh, NC 27610',
            address: '8558 Green Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-5.jpg', '/img/featured-img-6.jpg', '/img/featured-img-4.jpg', '/img/featured-img-2.jpg',]
        },
        {
            id: 6,
            title: 'RCE Theaters - 907 S Beckford Dr, Henderson, NC 27536',
            address: '8558 Pecan St.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-6.jpg', '/img/featured-img-3.jpg', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg',]
        },
    ],
    'Mumbai': [
        {
            id: 4,
            title: 'Costco Wholesale - 1021 Oak Forest Ln, Myrtle Beach, SC',
            address: '8558 Parker Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-4.jpg', '/img/featured-img-5.jpg', '/img/featured-img-6.jpg', '/img/featured-img-1.jpg',]
        },
        {
            id: 1,
            title: 'Regal North Hills - 4150 Main at North Hills St, Releigh',
            address: '3890 Poplar Dr.',
            rooms: 3,
            type: 'Sale',
            bed: 4,
            bath: 1,
            area: 732,
            price: '5,256',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-1.jpg']
        },
        {
            id: 2,
            title: 'Spring Lane Cinemas - 1351 Plaza Blvd, Sanford, NC 27330',
            address: '8558 Green Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-2.jpg', '/img/featured-img-3.jpg', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg',]
        },
        {
            id: 3,
            title: 'RCE Theaters - 907 S Beckford Dr, Henderson, NC 27536',
            address: '8558 Green Rd.',
            rooms: 3,
            type: 'Sale',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-3.jpg', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg', '/img/featured-img-6.jpg',]
        },
     
        {
            id: 5,
            title: 'Dollar General - 5416 Rock Quarry Rd, Raleigh, NC 27610',
            address: '8558 Green Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-5.jpg', '/img/featured-img-6.jpg', '/img/featured-img-4.jpg', '/img/featured-img-2.jpg',]
        },
        {
            id: 6,
            title: 'RCE Theaters - 907 S Beckford Dr, Henderson, NC 27536',
            address: '8558 Pecan St.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-6.jpg', '/img/featured-img-3.jpg', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg',]
        },
    ], 
    'Paris': [
        {
            id: 5,
            title: 'Dollar General - 5416 Rock Quarry Rd, Raleigh, NC 27610',
            address: '8558 Green Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-5.jpg', '/img/featured-img-6.jpg', '/img/featured-img-4.jpg', '/img/featured-img-2.jpg',]
        },
        {
            id: 1,
            title: 'Regal North Hills - 4150 Main at North Hills St, Releigh',
            address: '3890 Poplar Dr.',
            rooms: 3,
            type: 'Sale',
            bed: 4,
            bath: 1,
            area: 732,
            price: '5,256',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-1.jpg']
        },
        {
            id: 2,
            title: 'Spring Lane Cinemas - 1351 Plaza Blvd, Sanford, NC 27330',
            address: '8558 Green Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-2.jpg', '/img/featured-img-3.jpg', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg',]
        },
        {
            id: 3,
            title: 'RCE Theaters - 907 S Beckford Dr, Henderson, NC 27536',
            address: '8558 Green Rd.',
            rooms: 3,
            type: 'Sale',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-3.jpg', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg', '/img/featured-img-6.jpg',]
        },
        {
            id: 4,
            title: 'Costco Wholesale - 1021 Oak Forest Ln, Myrtle Beach, SC',
            address: '8558 Parker Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-4.jpg', '/img/featured-img-5.jpg', '/img/featured-img-6.jpg', '/img/featured-img-1.jpg',]
        },
    
        {
            id: 6,
            title: 'RCE Theaters - 907 S Beckford Dr, Henderson, NC 27536',
            address: '8558 Pecan St.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-6.jpg', '/img/featured-img-3.jpg', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg',]
        },
    ],
    'London': [
        {
            id: 6,
            title: 'RCE Theaters - 907 S Beckford Dr, Henderson, NC 27536',
            address: '8558 Pecan St.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-6.jpg', '/img/featured-img-3.jpg', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg',]
        },
        {
            id: 4,
            title: 'Costco Wholesale - 1021 Oak Forest Ln, Myrtle Beach, SC',
            address: '8558 Parker Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-4.jpg', '/img/featured-img-5.jpg', '/img/featured-img-6.jpg', '/img/featured-img-1.jpg',]
        },
        {
            id: 1,
            title: 'Regal North Hills - 4150 Main at North Hills St, Releigh',
            address: '3890 Poplar Dr.',
            rooms: 3,
            type: 'Sale',
            bed: 4,
            bath: 1,
            area: 732,
            price: '5,256',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-1.jpg']
        },
        {
            id: 2,
            title: 'Spring Lane Cinemas - 1351 Plaza Blvd, Sanford, NC 27330',
            address: '8558 Green Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-2.jpg', '/img/featured-img-3.jpg', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg',]
        },
        {
            id: 3,
            title: 'RCE Theaters - 907 S Beckford Dr, Henderson, NC 27536',
            address: '8558 Green Rd.',
            rooms: 3,
            type: 'Sale',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: true,
            img: ['/img/featured-img-3.jpg', '/img/featured-img-4.jpg', '/img/featured-img-5.jpg', '/img/featured-img-6.jpg',]
        },
     
        {
            id: 5,
            title: 'Dollar General - 5416 Rock Quarry Rd, Raleigh, NC 27610',
            address: '8558 Green Rd.',
            rooms: 3,
            type: 'Rent',
            bed: 4,
            bath: 1,
            area: 732,
            price: '7,255',
            favourite: false,
            popular: false,
            img: ['/img/featured-img-5.jpg', '/img/featured-img-6.jpg', '/img/featured-img-4.jpg', '/img/featured-img-2.jpg',]
        },
    
    ]
}